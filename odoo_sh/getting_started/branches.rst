...
If you merge a commit which is only changing some code without above mentioned modifications, then no backup is done 
by Odoo.sh, as neither the container nor the database are modified so the platform consider this safe enough. Of course, 
